---
title: "Data Check for Participants"
author: "Linda Betz"
...
library(scales)
set.seed(1)
data <- Questionnaire_Abfrage <- read_csv("Questionnaire_Abfrage.csv")
Study_Participants <- read_csv("Participants_Study_Time_Range.csv")
```
```{r, include=TRUE, echo=TRUE}
ID <- 20
```
We're looking at the participant with ID `r ID` with participant code
`r Study_Participants %>% filter(Participant_ID==ID) %>% select(Participant_Code)`.
...
least_fluctuation_plot <- data %>%
  mutate(date_ESM = as.Date(DateTime),
         time_ESM = DateTime) %>%
  filter(Participant_ID == ID) %>%
  rename_at(vars(matches("1-0")), ~ label_vars) %>%
  select(least_variable_items$Erlebnis, time_ESM) %>%
  pivot_longer(cols = least_variable_items$Erlebnis,
               names_to = "Erlebnis",
               values_to = "Wert") %>%
  ggplot(., aes(x = time_ESM, y = Wert, color = Erlebnis)) + facet_wrap(~
                                                                          Erlebnis, ncol = 1) +  geom_line(size =
...
```
## General Info
Participant `r ID` entered the PhenoNetz-Study on `r format(first_day, '%d %B, %Y')`. 
The last day in the study was `r format(last_day, '%d %B, %Y')`.
He/she completed `r number_days` days in the study.

## Raw Data 
```{r, echo = F}
data %>% filter(Participant_ID == ID) %>% print(n=Inf, width=Inf)
```
...
## Missingness
Participant `r ID` has `r round(missings,2)` % missing values.
...
## Fluctuation
The 3 most variable items were:
````{r}
most_variable_items
most_fluctuation_plot
```

The 3 least variable items were:
````{r}
least_variable_items
least_fluctuation_plot
```
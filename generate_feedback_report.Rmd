...
# some parameters to set: # minor check
alpha <-
  .01 # significance level for thresholding in networks

ID <- 13 # participant ID

smooth <- TRUE # should trend in time-series be included?
    ...
    tryCatch(
      expr = {
        model_pruned <- gvar(
          ...
          verbose = TRUE,
          estimator = "FIML"
        )  %>% runmodel %>% modelsearch(prunealpha = alpha, addalpha = alpha) 
        # prune(alpha = alpha, recursive = F, adjust = "none") %>% stepup(alpha = alpha)
        return(model_pruned)
      },
      error = function(e) {
        return(NULL)
...
fluctuation_plot <- data %>%
  ...
  scale_x_datetime(breaks =  date_breaks("2 days"), labels = date_format("%d.%m."))} else {
    most_variable_items <- data %>%
  mutate(date_ESM = as.Date(DateTime),
         time_ESM = DateTime) %>%
  filter(Participant_ID == ID) %>%

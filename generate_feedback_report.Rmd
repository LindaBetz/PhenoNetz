...
participant_code <- "UKK1VIZYAPYGTUXD" # participant code (in "")
    ...
    tryCatch(
      expr = {
        model_pruned <- gvar(
          ...
          verbose = TRUE,
          optimizer = "nlminb",
          estimator = "FIML") %>% runmodel %>% prune(alpha = alpha, recursive = F, adjust = "none", startreduce = 0.95) %>% stepup(alpha = alpha)

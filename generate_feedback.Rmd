...
participant_code <- "UKK1VIZYPEZHISKX" # participant code (in "")
    ...
    tryCatch(
      expr = {
        model_pruned <- gvar(
          ...
          standardize = "z",
          optimizer = "nlminb",
          estimator = "FIML") %>% runmodel %>% stepup(alpha = alpha) %>% psychonetrics::prune(alpha = alpha, recursive = F, adjust = "none", startreduce = 0.95)
